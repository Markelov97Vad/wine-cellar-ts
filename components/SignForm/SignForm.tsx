...
function SignForm({ register = false } : SignFormType) {
  ...
  return (
    <form onSubmit={handleSubmit} className={style.form}>
      ...
      {
        register ? 
        <span className={style.form__span}>Уже есть профиль? <Link className={style.form__link} href='/login'>Войти</Link></span> :
        <span className={style.form__span}>Новый пользователь? <Link className={style.form__link} href='/register'>Регистрация</Link></span>
      }
